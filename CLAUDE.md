...
### 浮层阅读器
- 点击卷卡片打开浮层
- 两种模式：「本卷总结」（Markdown 渲染）和「本卷原文」（竖排分页阅读）
- 原文行文从右到左；左方向键/PageDown 进入下一页，右方向键/PageUp 返回上一页；Mac 触摸板横向滑动每次只翻一页。
- 原文阅读器底部显示右到左页进度条和当前页码，可拖动进度条跳页；翻页按钮初始显示，翻页后仅在靠近左右翻页区域时显示。
- 朝代总览也有独立浮层
